...
class SettingsStore:
    ...
    def load_settings(self) -> tuple[list[Rule], list[str]]:
        if not self.path.exists():
            return [], sorted(DEFAULT_PROTECTED_DISK_IMAGE_EXTENSIONS)
    ...
    def load_rules(self) -> list[Rule]:
        rules, _extensions = self.load_settings()
        return rules

    def save_settings(self, rules: list[Rule], protected_disk_image_extensions: list[str]) -> None:
    ...
    def save_rules(self, rules: list[Rule]) -> None:
        self.save_settings(rules, sorted(DEFAULT_PROTECTED_DISK_IMAGE_EXTENSIONS))
